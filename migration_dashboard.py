...
# Migration bandwidth cap (MiB/s).
# At full burst a B->A migration can reach 400-900 Mbps on a 1 GbE lab network,
# starving the Xshell SSH session.  100 MiB/s ≈ 800 Mbps is a safe ceiling that
# completes a 2 GB VM in ~20 s while leaving 200 Mbps for SSH.
...
def _ensure_ssh_keepalive() -> None:
    """Write SSH keepalive dropin for all cluster hosts.

    libvirt's internal qemu+ssh connections (used by virsh -c qemu+ssh://)
    obey the system SSH client config.  Without ServerAliveInterval, a slow
    migration (bandwidth-capped or large VM) leaves the SSH control channel
    idle long enough for the remote sshd to time-out and drop it, which
    aborts the migration mid-transfer.

    The dropin is written to /etc/ssh/ssh_config.d/ so it applies to ALL
    SSH clients on Host-A — our _virsh calls AND libvirt's internal SSH.
    """
    dropin = Path("/etc/ssh/ssh_config.d/51-hw6-dashboard-keepalive.conf")
    try:
        ips    = [HOSTS_CONFIG[h]["ip"]   for h in HOSTS_CONFIG]
        fqdns  = [HOSTS_CONFIG[h]["fqdn"] for h in HOSTS_CONFIG]
        shorts = [HOSTS_CONFIG[h]["short"] for h in HOSTS_CONFIG]
        host_pat = " ".join(ips + fqdns + shorts)

        content = (
            "# HW6 dashboard — auto-generated by migration_dashboard.py\n"
            "# Keeps SSH control channels alive during slow/bandwidth-capped migrations.\n"
            "# Safe to delete; dashboard recreates it on next start.\n"
            f"Host {host_pat}\n"
        ...
        pass   # not root / read-only fs — keepalive will rely on MIG_BANDWIDTH_MIBPS only
...
def main():
    ...
    # Write SSH keepalive dropin — applies to libvirt qemu+ssh AND our _virsh calls
