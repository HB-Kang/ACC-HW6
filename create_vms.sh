...
if [[ "$do_place" -eq 1 ]]; then
    ...
    hw6_migration_preflight serverc || warn "Host-C preflight failed — fix NFS/FQDN on C (see HW6_Sub.md)"

    for vm in vm-3 vm-4; do
        info "  $vm → Host-B (serverb)..."
        hw6_virsh_migrate_live "$vm" "$(hw6_migrate_dest_uri serverb)" serverb && ok "$vm → Host-B done"
    done
    for vm in vm-5 vm-6; do
        info "  $vm → Host-C (serverc)..."
        hw6_virsh_migrate_live "$vm" "$(hw6_migrate_dest_uri serverc)" serverc && ok "$vm → Host-C done"
    done
fi
